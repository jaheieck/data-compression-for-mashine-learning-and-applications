...
% the QMCVoronoi. The evaluation will be done over 100 random samples and 
...
for z = 1:number
    ...
    err_QMCVor_continuous_integrand = err_QMCVor_continuous_integrand + 1/size(P_continuous_integrand,2)*sum((exp(-sum(a.*abs(P_continuous_integrand-u),1))-Yp_continuous_integrand(1,:)).^2);
    ...
    % err_QMCVor_corner_peak_integrand = err_QMCVor_corner_peak_integrand + 1/size(P_corner_peak_integrand,2)*sum(((1+sum(a.*P_corner_peak_integrand,1)).^(-s-1)-Yp_corner_peak_integrand(1,:)).^2);
    ...
    err_QMCVor_discontinuous_integrand = err_QMCVor_discontinuous_integrand + 1/size(P_discontinuous_integrand,2)*sum((discontinuousintegrand(P_discontinuous_integrand,a,u)-Yp_discontinuous_integrand(1,:)).^2);
    ...
    % err_QMCVor_oscillatory_integrand = err_QMCVor_oscillatory_integrand + 1/size(P_oscillatory_integrand,2)*sum((cos(2*pi*u(1)+sum(a.*P_oscillatory_integrand,1))-Yp_oscillatory_integrand(1,:)).^2);

    % % G-Function
    % err_Gfct = err_Gfct + 1/N*sum((G_fct(X)-Y_Gfct).^2);
    % err_supercom_Gfct = err_supercom_Gfct + 1/K*sum((G_fct(x_Gfct)-y_Gfct).^2);
    % err_QMCVor_Gfct = err_QMCVor_Gfct + 1/size(P_Gfct,2)*sum((G_fct(P_Gfct)-Yp_Gfct(1,:)).^2);
    % 
    % % Morokoff & Caflisch function 1
    % err_MC = err_MC + 1/N*sum(((1+1/s)^s*prod(X.^(1/s),1)-Y_MC).^2);
    % err_supercom_MC = err_supercom_MC + 1/K*sum(((1+1/s)^s*prod(x_MC.^(1/s),1)-y_MC).^2);
    % err_QMCVor_MC = err_QMCVor_MC + 1/size(P_MC,2)*sum(((1+1/s)^s*prod(P_MC.^(1/s),1)-Yp_MC(1,:)).^2);
    % 
    % % Bratley function
    ...
    % err_QMCVor_Bratley = err_QMCVor_Bratley + 1/size(P_Bratley,2)*sum((Bratley_fct(P_Bratley)-Yp_Bratley(1,:)).^2);

    % Zhou function
    err_Zhou = err_Zhou + 1/N*sum((Zhou_fct(X)-Y_Zhou).^2);
    err_supercom_Zhou = err_supercom_Zhou + 1/K*sum((Zhou_fct(x_Zhou)-y_Zhou).^2);
    err_QMCVor_Zhou = err_QMCVor_Zhou + 1/size(P_Zhou,2)*sum((Zhou_fct(P_Zhou)-Yp_Zhou(1,:)).^2);
    ...
    %     err_QMCVor_environmental = err_QMCVor_environmental + 1/size(P_environmental,2)*sum((environmental_fct(P_environmental,M,D,L,tau)-Yp_environmental(1,:)).^2);
    % end
end
...
save(sprintf('err_QMCVor_%d_%d_%d.mat',[N s K]),'err')


%% Output we are not iterested in in this analysis
% accuracy_supercom_corner_peak_integrand  accuracy_supercom_oscillatory_integrand  accuracy_supercom_Gfct  accuracy_supercom_MC  accuracy_supercom_Bratley  
% accuracy_supercom_environmental accuracy_QMCVor_corner_peak_integrand  accuracy_QMCVor_oscillatory_integrand  accuracy_QMCVor_Gfct  accuracy_QMCVor_MC  
% accuracy_QMCVor_Bratley  accuracy_QMCVor_environmental